...
export default function FiscalPage() {
  ...
  const CardFiscal = ({ label, credito, debito, saldo }: { label: string; credito: number; debito: number; saldo: number }) => (
    <div className="card p-4">
      <div className="text-sm text-muted font-medium mb-3">{label}</div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="card p-4">
              <p className="text-sm text-muted mb-1">Total compras (entradas)</p>
              <p className="text-2xl font-bold text-gray-800">{moeda(somaEntradas.produtos)}</p>
              <p className="text-xs text-muted mt-1">{entradasMes.length} NF(s) de entrada no mês</p>
            </div>
            <div className="card p-4">
              <p className="text-sm text-muted mb-1">Total vendas expedidas (saídas)</p>
              <p className="text-2xl font-bold text-gray-800">{moeda(somaSaidas.produtos)}</p>
              <p className="text-xs text-muted mt-1">{saidasMes.length} NF(s) de saída no mês</p>
            </div>
          </div>
          ...
          <div className="card mb-6">
            ...
            {entradasMes.length === 0 ? (
              <p className="p-4 text-center text-muted text-sm">Nenhuma NF de entrada neste mês.</p>
            ) : (
              <table className="w-full text-sm">
                <thead className="bg-page">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-right text-muted font-medium">Produtos</th>
                    <th className="px-4 py-2 text-right text-tax-icms font-medium">ICMS Créd.</th>
                    <th className="px-4 py-2 text-right text-primary font-medium">IPI Créd.</th>
                    <th className="px-4 py-2 text-right text-tax-pis font-medium">PIS/COFINS Créd.</th>
                  </tr>
                </thead>
                <tbody>
                  {entradasMes.map(n => (
                    <tr key={n.id} className="border-t hover:bg-page">
                      ...
                      <td className="px-4 py-2 text-muted text-xs">{n.tipo_entrada.replace(/_/g, " ")}</td>
                      <td className="px-4 py-2 text-right">{moeda(n.valor_total_produtos || 0)}</td>
                      <td className="px-4 py-2 text-right text-tax-icms">{moeda(n.valor_icms_credito || 0)}</td>
                      <td className="px-4 py-2 text-right text-primary">{moeda(n.valor_ipi_credito || 0)}</td>
                      <td className="px-4 py-2 text-right text-tax-pis">{moeda((n.valor_pis_credito || 0) + (n.valor_cofins_credito || 0))}</td>
                    </tr>
                  ))}
                  <tr className="border-t bg-page font-semibold">
                    <td colSpan={2} className="px-4 py-2 text-gray-600">Total</td>
                    <td className="px-4 py-2 text-right">{moeda(somaEntradas.produtos)}</td>
                    <td className="px-4 py-2 text-right text-tax-icms">{moeda(somaEntradas.icms)}</td>
                    <td className="px-4 py-2 text-right text-primary">{moeda(somaEntradas.ipi)}</td>
                    <td className="px-4 py-2 text-right text-tax-pis">{moeda(somaEntradas.pis + somaEntradas.cofins)}</td>
                  </tr>
                </tbody>
              </table>
            )}
          ...
          <div className="card">
            ...
            {saidasMes.length === 0 ? (
              <p className="p-4 text-center text-muted text-sm">Nenhuma NF de saída neste mês.</p>
            ) : (
              <table className="w-full text-sm">
                <thead className="bg-page">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-right text-muted font-medium">Produtos</th>
                    <th className="px-4 py-2 text-right text-tax-icms font-medium">ICMS Déb.</th>
                    <th className="px-4 py-2 text-right text-primary font-medium">IPI Déb.</th>
                    <th className="px-4 py-2 text-right text-tax-pis font-medium">PIS/COFINS Déb.</th>
                    <th className="px-4 py-2 text-right text-muted font-medium">Total NF</th>
                  </tr>
                </thead>
                <tbody>
                  {saidasMes.map(n => (
                    <tr key={n.id} className="border-t hover:bg-page">
                      ...
                      <td className="px-4 py-2 text-muted">{new Date(n.data_emissao).toLocaleDateString("pt-BR")}</td>
                      <td className="px-4 py-2 text-right">{moeda(n.valor_produtos || 0)}</td>
                      <td className="px-4 py-2 text-right text-tax-icms">{moeda(n.valor_icms || 0)}</td>
                      <td className="px-4 py-2 text-right text-primary">{moeda(n.valor_ipi || 0)}</td>
                      <td className="px-4 py-2 text-right text-tax-pis">{moeda((n.valor_pis || 0) + (n.valor_cofins || 0))}</td>
                  ...
                  <tr className="border-t bg-page font-semibold">
                    ...
                    <td className="px-4 py-2 text-right text-tax-icms">{moeda(somaSaidas.icms)}</td>
                    <td className="px-4 py-2 text-right text-primary">{moeda(somaSaidas.ipi)}</td>
                    <td className="px-4 py-2 text-right text-tax-pis">{moeda(somaSaidas.pis + somaSaidas.cofins)}</td>
